                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <a href="{{ route('surveyings.create') }}" class="btn btn-success btn-sm">
                            ...
                            <div class="card-body">
                                <table id="adminLteDataTable" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @forelse($surveyings as $surveying)
                                            <tr data-entry-id="{{ $surveying->id }}">
                                                <td>{{ $loop->iteration }}</td>
                                                <td><strong>{{ $surveying->equipment }}</strong><br>{{ $surveying->description }}
                                                </td>
                                                <td>{{ $surveying->brand }}</td>
                                                <td>{{ $surveying->quantity }}</td>
                                                <td>{{ $surveying->unit }}</td>
                                                <td>{{ date('F d, Y', strtotime($surveying->date_acquired)) }}</td>
                                                <td>

                                             
                                                    <div class="btn-group btn-group-sm" role="group"
                                                        aria-label="Basic example">
                                                        <a href="{{ route('surveyings.show', ['surveying' => $surveying->id]) }}"
                                                            class="btn btn-info bg-blue-500 py-2 rounded-lg mr-2 text-white">View
                                                            More</a>
                                                        <form action="{{ route('surveyings.edit', ['surveying' => $surveying->id]) }}">
                                                            <button class="btn btn-secondary">
                                                                <i class="fa fa-pencil-alt"></i>
                                                            </button>
                                                        </form>
                                                        &nbsp
                                                        <form action="{{ route('surveyings.destroy', $surveying->id) }}"
